...
SAMPLING_RATE = 16000
BATCH_SIZE = 8  # Number of files to process in a batch
...
def process_audio_file(audio_path):
    try:
        ...
        print(f"Loaded audio: {audio_path} | Original Sample Rate: {orig_sr} | Audio Shape: {audio_array.shape}")

        audio_resample = librosa.resample(audio_array, orig_sr=orig_sr, target_sr=SAMPLING_RATE)
        print(f"Resampled audio: {audio_path} | Resampled Shape: {audio_resample.shape}")
...
output_directory = r"C:\program1\my_project\tmp" 
input=r'C:\program1\my_project\samples\output.wav'
print('start--------------------------------------------------')
print(process_audio_file(input))