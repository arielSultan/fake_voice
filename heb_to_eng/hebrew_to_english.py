...
import os
from files_nakdimon import add_niqqud
            ...
            if i < 1:
                ...
                if list_num[0] != 0:
                    if list_num[1] + list_num[2] < 20:
                        list_heb.append(
                            "וְ" + num_dict_below_20[list_num[1] + list_num[2]]
                        )
                        break
                    else:
                        list_heb.append(num_dict_eq_above_20[num])
...
def NumberToHebrew(number):
    ...
    signs_dict = {"%": "אָחוּז", ",": "פְּסִיק", ".": "נְקֻדָּה"}
    ...
    num_dict_below_20 = {
        ...
        13: "שְׁלוֹשׁ עֶשְׂרֵה",
        14: "אַרְבַּע עֶשְׂרֵה",
        15: "חָמֵשׁ עֶשְׂרֵה",
        16: "שֵׁשׁ עֶשְׂרֵה",
        17: "שֶׁבַע עֶשְׂרֵה",
        18: "שְׁמוֹנֶה עֶשְׂרֵה",
        19: "תֵּשַׁע עֶשְׂרֵה",
    }
    ...
    if number < 10000:
                ...
                if i < 2:
                    ...
                    if list_num[0] != 0 or list_num[1] != 0:
                        if list_num[2] + list_num[3] < 20:
                            list_heb.append(
                                "וְ" + num_dict_below_20[list_num[2] + list_num[3]]
                            )
                            break
                        else:
                            list_heb.append(num_dict_eq_above_20[num])
                        ...
                        if list_num[2] + list_num[3] < 20:
                            list_heb.append(
                                num_dict_below_20[list_num[2] + list_num[3]]
                            )
                            break
                        else:
                            list_heb.append(num_dict_eq_above_20[num])
        ...
        ret_list, flag_zero = build_three_num_heb(
            s1, num_dict_below_20, num_dict_eq_above_20, False
        )
        if not flag_zero:
        ...
        ret_list, flag_zero = build_three_num_heb(
            s2, num_dict_below_20, num_dict_eq_above_20, False
        )
        if not flag_zero:
            for item in ret_list:
                list_heb.append(item)
        ...
        ret_list, flag_zero = build_three_num_heb(
            s3, num_dict_below_20, num_dict_eq_above_20, True
        )
        if not flag_zero:
            for item in ret_list:
                list_heb.append(item)
...
def is_pattern_number_with_heb(string):
    ...
    if (
        not parts[1].isdigit()
        and not is_number_range(parts[1])
        and not is_number_with_comma(parts[1])
        and not is_number_with_decimal(parts[1])
        and not is_percentage(parts[1])
    ):
        return False
...
def clean_pattern_number_with_heb(string):
    ...
    if len(parts) <= 2:
        ...
        if is_number_with_comma(parts[1]):
            return (
                parts[0],
                str(clean_number_with_comma(parts[1])),
...
# ----------------------------------------------------------------------
def is_number_with_quote(string):
...
def clean_number_with_quote(string):
    if "," in string or "." in string:
        if string[-1] != ",":
            return None
        string = string[:-1]
...
def is_number_with_dots(string):
    if string[-1] == "," or string[-1] == ".":
        string = string[:-1]
    if "." not in string:
        return False
...
def clean_number(word):
    ...
    ## "6'8" / "5'7" / "6'0"
    if is_number_with_quote(word):
        list_heb = []
        part1, part2 = clean_number_with_quote(word)
        list_heb += NumberToHebrew(part1)
        list_heb += NumberToHebrew(part2)
        print(list_heb)
        return list_heb
    ## "43.5.7" / "1.23.5" / "1.2.2.45"
    if is_number_with_dots(word):
        numbers = clean_numbers_dots(word)
        list_heb = []
        for num in numbers:
            list_heb += NumberToHebrew(num)
        return list_heb
    ## '2.9' / '3.4'
    ...
    ## '4.5%' / '9.25%.' / '26.5%,'
        ...
        if part2 != None:
            ...
            list_heb += NumberToHebrewMale(part1)
            list_heb += NumberToHebrewMale("%")
            return list_heb
    ...
    ## unknown
    special_signs = "!@#$%^&*()?><'_-=+"
    if any(char in word for char in special_signs):
        print(word)
        list_heb = []
        splitted_strings = re.split(f"[{re.escape(special_signs)}]", word)
        splitted_strings = [item for item in splitted_strings if item]
        for string in splitted_strings:
            string = re.sub(
                r"(?<=\d)(?=[א-תA-Za-z])|(?<=[א-תA-Za-z])(?=\d)", "-", string
            )
            list_heb += clean_number(string)
        return list_heb
...
def turn_letter_to_word(letter):
    letters_dict = {
        "א'": "אַלֶף",
        "ב'": "בֵּית",
        "ג'": "גִּימֶל",
    ...
    for word in words:
        ...
        if not has_number(word):
            ...
            for brk_word in broken_word:
                ...
                if brk_word == "." or brk_word == "," or brk_word == ";":
                    new_sentence += brk_word + " "

                else:
                    pattern = re.compile(r"^[\u0590-\u05FF]\'$")
                    if bool(pattern.match(brk_word)):
                        brk_word = turn_letter_to_word(brk_word)
        ...
        ##if there is a number:
            ...
            try:
                ...
                if has_number(after_num) or has_number(before_num) or after_num == "%":
                ...
                print(
                    "error from split_number_and_string in line:",
                    index,
                    "with word: ",
                    word,
                )
...
def dup_folder_english(input_folder, output_folder):
    dirs = get_directory_paths(input_folder)
    hebrew_files_to_english(input_folder, output_folder)
    for i in dirs:
        hebrew_files_to_english(
            os.path.join(input_folder, i), os.path.join(output_folder, i)
        )


def hebrew_files_to_english(input_folder, output_folder):
    ...
    for file in files:
        with open(file, "r", encoding="utf-8") as currentFile:
            ...
            mnukad = add_niqqud(content)
            try:
                englishText = HebrewToEnglish(mnukad)
            except:
                print(file)
            with open(output_file_path, "w", encoding="utf-8") as output:
                output.write(englishText)


print(HebrewToEnglish(add_niqqud(input("enter text: "))))
# print(HebrewToEnglish((input("enter text: "))))